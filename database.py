...
class Database:
    ...
    def first_page(self):
        ...
        self.curse.execute(
            "SELECT * FROM WEEKS WHERE name = '" + self.name + "'")  # see if we already added the recent week
    ...
    def insert(self, songlist, videos, titles):  # adds the songs to the new table
        ...
        if len(songlist) == len(videos):
            size = len(songlist)
            for i in range(size):
                insert_query = "INSERT INTO SONGS VALUES (" + str(i + 1) + ",\'" + songlist[i] + "\',\'" + videos[
                    i] + "\',\'" + titles[i].replace("'", "") + "\',\'" + self.name + "\');"
