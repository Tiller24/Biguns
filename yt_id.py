...
if "YOUTUBE_API_KEY" in os.environ:
	DEVELOPER_KEY = os.environ.get("YOUTUBE_API_KEY") 								# the youtube api key, stored in environment variable. Need your own to run!
...
class yt_id:
	...
	def youtube_search(self,query,max):
		...
		search_response = self.youtube.search().list(								# Call the search.list method to retrieve results matching the specified
		q = query,																	# query term.
		part = "id,snippet",
		maxResults = max
		).execute()
